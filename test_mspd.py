...
from measure import compute_psnr, compute_ssim, compute_rmse, compute_lpips
...
def test(opts, model, device):
    ...
    input_psnr, input_ssim, input_rmse, input_lpips = 0.0, 0.0, 0.0, 0.0
    test_psnr, test_ssim, test_rmse, test_lpips = 0.0, 0.0, 0.0, 0.0

    windeowed_input_psnr, windeowed_input_ssim, windeowed_input_rmse = 0.0, 0.0, 0.0
    windeowed_pred_psnr, windeowed_pred_ssim, windeowed_pred_rmse = 0.0, 0.0, 0.0
    ...
    with torch.no_grad():
        for iter_, (noisy, target) in enumerate(test_loader):
            ...
            _input_rmse = compute_rmse(target, noisy)
            ...
            _windowed_input_rmse = compute_rmse(windowed_target, windowed_input)
        ...
        input_rmse_avg = input_rmse / len(test_dataset)
        ...
        test_rmse_avg = test_rmse / len(test_dataset)
        ...
        windowed_input_rmse_avg = windeowed_input_rmse / len(test_dataset)

        windowed_pred_psnr_avg = windeowed_pred_psnr / len(test_dataset)
        windowed_pred_ssim_avg = windeowed_pred_ssim / len(test_dataset)
        windowed_pred_rmse_avg = windeowed_pred_rmse / len(test_dataset)

        print('\n')
        print('Original === \nPSNR avg: {:.4f} \nSSIM avg: {:.4f} \nRMSE avg: {:.4f} \nLPIPS avg: {:.4f}'.format(input_psnr_avg, input_ssim_avg, input_rmse_avg, input_lpips_avg))
        print('\n')
        print('Predictions === \nPSNR avg: {:.4f} \nSSIM avg: {:.4f} \nRMSE avg: {:.4f} \nLPIPS avg: {:.4f}'.format(test_psnr_avg, test_ssim_avg, test_rmse_avg, test_lpips_avg))
        print('\n')
        print('Original === \nPSNR avg: {:.4f} \nSSIM avg: {:.4f} \nRMSE avg: {:.4f}'.format(windowed_input_psnr_avg, windowed_input_ssim_avg, windowed_input_rmse_avg))
        print('\n')
        print('Predictions === \nPSNR avg: {:.4f} \nSSIM avg: {:.4f} \nRMSE avg: {:.4f}'.format(windowed_pred_psnr_avg, windowed_pred_ssim_avg, windowed_pred_rmse_avg))
        print('\n')
        ...
        with open(os.path.join(opts.result_path, 'average_results.txt'), 'w') as avg_file:
            ...
            avg_file.write(f"PSNR: {input_psnr_avg}, SSIM: {input_ssim_avg}, RMSE: {input_rmse_avg}, LPIPS: {input_lpips_avg}\n\n")
            avg_file.write('Predicted average results:\n')
            avg_file.write(f"PSNR: {test_psnr_avg}, SSIM: {test_ssim_avg}, RMSE: {test_rmse_avg} LPIPS: {test_lpips_avg}\n")
            avg_file.write('Original average results:\n')
            avg_file.write(f"PSNR: {windowed_input_psnr_avg}, SSIM: {windowed_input_ssim_avg}, RMSE: {windowed_input_rmse_avg}\n")
            avg_file.write('Predicted average results:\n')
            avg_file.write(f"PSNR: {windowed_pred_psnr_avg}, SSIM: {windowed_pred_ssim_avg}, RMSE: {windowed_pred_rmse_avg}\n")
